...
# susieI (filtered regions) - setting 1
...
# susieI (filtered regions) - setting 2

* Regions: We use regional PIP of both SNP and gene from mr.ash2s > 0.3.

* Susie run parameters: `L=2`, initialize `null_weight = 0`, update `null_weight` in iterations. We initialize with prior for genes and SNPs as uniform.

## One example

Prior change for each iteration:
```{r iter-6}
load(paste0(outputdir, tag, ".config6.susieIres.Rd"))
df <- cbind(prior.gene_rec, prior.SNP_rec)
rownames(df) <- paste("Iteration", 1:Niter)
colnames(df) <- c("Prior.gene", "Prior.SNP")
df
...
# susieI (filtered regions) - setting 3

* Regions: We use regional PIP of both SNP and gene from mr.ash2s > 0.3.

* Susie run parameters: `L=2`, initialize `null_weight = 0`, update `null_weight` in iterations. We initialize with prior for genes and SNPs as 0.05 and 2.5e-3.

## One example

Prior change for each iteration:
```{r iter-8}
load(paste0(outputdir, tag, ".config8.susieIres.Rd"))
df <- cbind(prior.gene_rec, prior.SNP_rec)
rownames(df) <- paste("Iteration", 1:Niter)
colnames(df) <- c("Prior.gene", "Prior.SNP")
df
...
```{r param-8}
tags <- paste(rep(1:4, each = 5), 1:5, sep = "-")
susieIfs <- paste0(outputdir, "20200813-", tags, ".config8.susieIres.Rd")
susieIfs2 <- paste0(outputdir, "20200813-", tags, ".config8.susieI.txt")

show_param(mrashfs, susieIfs, susieIfs2)
```

# susieI (filtered regions) - setting 4
...
Prior change for each iteration:
```{r iter-7}
load(paste0(outputdir, tag, ".config7.susieIres.Rd"))
df <- cbind(prior.gene_rec, prior.SNP_rec)
rownames(df) <- paste("Iteration", 1:Niter)
colnames(df) <- c("Prior.gene", "Prior.SNP")
df
