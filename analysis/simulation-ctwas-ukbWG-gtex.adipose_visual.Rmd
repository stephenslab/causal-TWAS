...
plot_all <- function(susief, pf, phenof, chr, colocf = NULL,  ldres = NULL, thin = 1){
  ...
  if (!is.null(colocf)) {
    a <- fread(colocf, header = T)
    setnames(a, old = "ID", new = "name")
    dt.gene <- merge(dt.gene, a, by ="name")
  }
  ...
  if (is.null(colocf)) {
     ...
     fig7 <- plot_coloc_pp4(dt.gene)
     fig <- subplot(fig1, fig3, fig4, fig6, fig7, nrows=5, shareX = T, titleY = T)
...
under 4-1 to 4-5
```{r sim GN, out.width = "98%", out.height="95%"}
chrom <- 4
runtag <- 4
...
```{r sim GA, out.width = "98%", out.height="95%"}
runtag <- 5
chrom <- 1
plot_all(susiefs[runtag], pfs[runtag], phenofs[runtag], chrom, thin = 0.01)
```
...
```{r sim 10-1, out.width = "98%", out.height="95%"}
runtag <- 5
chrom <- 1
plot_all(susiefs[runtag], pfs[runtag], phenofs[runtag], chrom, thin = 0.01)
```
...
```{r sim c1, out.width = "98%", out.height="95%"}
runtag <- 4
chrom <- 1
plot_all(susiefs[runtag], pfs[runtag], phenofs[runtag], chrom, thin = 0.01)
```

# region plot with coloc results
```{r sim 3, out.width = "98%", out.height="95%"}
runtag <- "ukb-s80.45-adi"
i <- 4
chrom <- 5
comparedir <- "/project2/mstephens/causalTWAS/simulations/simulation_ctwas_rss_20210416_compare/"
colocfs <- paste0(comparedir, runtag, "_simu", simutags,".Adipose_Subcutaneous.coloc.result")
plot_all(susiefs[i], pfs[i], phenofs[i], chrom, colocf = colocfs[i], thin = 0.01)
```
## Zoom view
For new simulation settings in 20230322
False positives
```{r sim 4, out.width = "98%", out.height="95%"}
outputdir <- "~/causalTWAS/simulations/simulation_ctwas_rss_20230322/"
runtag <- "ukb-s80.45-adi"
...
## Results from `L=1` vs `L=5` for low SNP PVE settings.

```{r sim 2, out.width = "98%", out.height="95%"}
simuoutputdir <- "~/causalTWAS/simulations/simulation_ctwas_rss_20230322/"
runtag <- "ukb-s80.45-adi"
simutags <- paste(8, 1, sep = "-")
...
```
Zoom in view
```{r L=5, out.width = "98%", out.height="95%"}
configtag <- 1
susiefs <- paste0(outputdir, runtag, "_simu", simutags, "_config", configtag, ".susieIrss.txt")
pfs <- paste0(outputdir, runtag, "_simu",simutags)
gwasfs <- paste0(pfs, ".exprgwas.txt.gz")
...
chrom <- 17
rtag <- 1
plot_all(susiefs[rtag], pfs[rtag], phenofs[rtag], chrom, thin = 0.01)
```
```{r L=1, out.width = "98%", out.height="95%"}
configtag <- 2
susiefs <- paste0(outputdir, runtag, "_simu", simutags, "_config", configtag, ".susieIrss.txt")
pfs <- paste0(outputdir, runtag, "_simu",simutags)
gwasfs <- paste0(pfs, ".exprgwas.txt.gz")
...
exprfn = "/home/simingz/causalTWAS/simulations/simulation_ctwas_rss_20230322//ukb-s80.45-adi.expr.txt"
...
configtag <- 1
# debug(susieI_rss) to generate data. save(list=ls(), file="~/temp.Rd")
a <- plot_region(runtag, simutags[1], configtag, chr = 17, startpos = 9965921, endpos = 12534769,  plot_ceqtl = F, rerun_ctwas = T)
...
susieres <- susie_rss(z, R, z_ld_weight = z_ld_weight, 
                  ...
                  coverage = coverage)
outdf.reg <- ctwas:::anno_susie(susieres, geneinfo, snpinfo, gidx, sidx , 1, 1) 
```
