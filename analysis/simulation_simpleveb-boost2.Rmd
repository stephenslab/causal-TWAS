...
source("~/causalTWAS/causal-TWAS/code/mr.ash2.R")
```
...
```{r run mr.ash}
myorder <- 1:dim(X)[2]
myorder[gene.idx] <- 1:length(gene.idx)
myorder[SNP.idx] <- (length(gene.idx) + 1): length(myorder)

X <- cbind(X[, gene.idx], X[, SNP.idx])
fit <- mr.ash(X, y, method="caisa", update.order = NULL)
summary_mr.ash(fit)
...
```{r run mr.ash}
myorder <- 1:dim(X)[2]
myorder[gene.idx] <- (length(SNP.idx) + 1): length(myorder)
myorder[SNP.idx] <- 1:length(SNP.idx)
X <- cbind(X[,SNP.idx], X[,gene.idx])
fit2 <- mr.ash(X, y, method="caisa", update.order = NULL)
summary_mr.ash(fit2)

plot_beta(beta[gene.idx], fit2$beta[gene.idx], main = "beta for gene effect")
plot_beta(beta[SNP.idx], fit2$beta[SNP.idx], main = "beta for SNP effect")
```
...
fit <- mr.ash2s(X.gene, X.SNP, y, init.order = "expr-snp")
...
```{r run mr.ash2, snp}
fit <- mr.ash2s(X.gene, X.SNP, y, init.order = "snp-expr")
...
```{r run mr.ash2, lasso}
fit <- mr.ash2s(X.gene, X.SNP, y, mr.ash.init = "lasso")
...
```{r run mr.ash2, lassosnp}
fit <- mr.ash2s( X.gene, X.SNP, y, mr.ash.init = "lassoSNP")
