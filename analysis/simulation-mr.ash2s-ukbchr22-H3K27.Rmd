...
We use weights from Xiaotong, H3K27ac (trained using FUSION). Total features: > 8,000. on chromosome 22: 296.
...
```{r output dir}
simdatadir <- "~/causalTWAS/simulations/simulation_ashtest_20200615/"
outputdir <- "~/causalTWAS/simulations/simulation_ashtest_20200615/"
```

## Simulation 1
True parameters:
```{r para 1}
load(paste0(simdatadir, "simu_20200615-1-pheno.Rd"))
readLines(paste0(simdatadir, "param-20200615-1.R"))
```

MR.ASH2s results (start from gene):
```{r sim 1 gene, out.width = "98%", out.height="95%"}
load(paste0(outputdir, "20200615-1-mr.ash2s.expr-res.Rd"))
...
```{r sim 1 snp, eval = F}
load(paste0(outputdir, "20200615-1-mr.ash2s.snp-res.Rd"))
...
True parameters:
```{r para 2}
load(paste0(simdatadir, "simu_20200615-2-pheno.Rd"))
readLines(paste0(simdatadir, "param-20200615-2.R"))
```

MR.ASH results (start from gene):
```{r sim 2 gene, out.width = "98%", out.height="95%"}
load(paste0(outputdir, "20200615-2-mr.ash2s.expr-res.Rd"))
g.fit <- mr.ash2s.fit$fit1
s.fit <-  mr.ash2s.fit$fit2
summary_mr.ash2(g.fit, s.fit, phenores)
...
True parameters:
```{r para 3}
load(paste0(simdatadir, "simu_20200615-3-pheno.Rd"))
readLines(paste0(simdatadir, "param-20200615-3.R"))
```

MR.ASH results (start from gene):
```{r sim 3 gene, out.width = "98%", out.height="95%"}
load(paste0(outputdir, "20200615-3-mr.ash2s.expr-res.Rd"))
g.fit <- mr.ash2s.fit$fit1
s.fit <-  mr.ash2s.fit$fit2
summary_mr.ash2(g.fit, s.fit, phenores)
...
True parameters:
```{r para 4}
load(paste0(simdatadir, "simu_20200615-4-pheno.Rd"))
readLines(paste0(simdatadir, "param-20200615-4.R"))
```

MR.ASH results (start from gene):
```{r sim 4 gene, out.width = "98%", out.height="95%"}
load(paste0(outputdir, "20200615-4-mr.ash2s.expr-res.Rd"))
g.fit <- mr.ash2s.fit$fit1
s.fit <-  mr.ash2s.fit$fit2
summary_mr.ash2(g.fit, s.fit, phenores)
