...
caliPP4_plot <- function(phenofs, colocfs, main = "coloc PP4 Calibration"){
  cau <- lapply(phenofs, function(x) {load(x);get_causal_id(phenores)})
  df <- NULL
...
ncausalFUSIONp_plot <- function(phenofs, fusionfs, main = "FUSION FDP"){
  ...
  fig <- nca_plot(df$FDR, df$ifcausal, df$runtag, mode ="FDR", main = main)
  return(fig)
}

ncausalPP4_plot <- function(phenofs, colocfs, main = "coloc PP4"){
  ...
  fig <- nca_plot(df$COLOC.PP4, df$ifcausal, df$runtag, mode ="PIP", main = main)
  return(fig)
}
