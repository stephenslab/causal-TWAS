...
### SUSIE results: PIP > 0.3 or causal genes

```{r sim 5 examples, include= F, echo = F, eval = F, warning= F}
# displaying not right
resall <- Sys.glob(paste0(outputdir, tag, ".", tag2, "*-susieres.txt"))
l <- htmltools::tagList()
res <- resall[1]
for (i in 1:8){
  res <- resall[i]
...
# displaying not right
res <- paste0(outputdir, tag, ".", tag2, "-LL22NC03-86G7.1-susieres.txt")
name <- gsub(".*res-(.+)-susie.*", "\\1", res)
