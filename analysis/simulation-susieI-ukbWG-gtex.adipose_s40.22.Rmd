...
* Susie run parameters: `L=1`.  We initialize with prior for genes and SNPs as uniform. gene.pve ~ 0.1, snp.pve ~ 0.5.  Null weight is calculated based on prior of genes and SNPs ( 1 - sum of priors for snps and genes). 
...
* Susie run parameters: `L=1`. We initialize with prior for genes and SNPs using their true values. gene.pve ~ 0.1, snp.pve ~ 0.5.  Null weight is calculated based on prior of genes and SNPs ( 1 - sum of priors for snps and genes). 
...
* Susie run parameters: `L=1`. initialize with `null_weight = 0` and update `null_weight` based on last iteration results. We initialize with prior for genes and SNPs as uniform. gene.pve ~ 0.1, snp.pve ~ 0.5. 
...
```{r gene twas, fig.height=4, fig.width=10}
chrom <- 1
tag <- "20201001-2-1"
pipfile <-  paste0(outputdir, tag, ".config2.1.SERI.txt")
a <- fread(pipfile, header = T)

plot(a[a$chr ==chrom, ]$pos, a[a$chr ==chrom, ]$SERpip, col = a[a$chr ==chrom, ]$ifcausal + 1, xlab = paste0("chr", chrom), ylab = "pip", pch = c(1,2)[as.factor(a[a$chr ==chrom, ]$type)])
points(a[a$chr ==chrom & a$ifcausal ==1, ]$pos, a[a$chr ==chrom & a$ifcausal ==1, ]$SERpip , col = "red", pch = c(19, 17)[as.factor(a[a$chr ==chrom & a$ifcausal ==1 , ]$type)])
legend("topright",c("gene","SNP"),cex=.8,pch=c(1,2))
grid()
```
